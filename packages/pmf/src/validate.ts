...
import type { PMFFile, ValidationResult } from "./types.js";
...
export function validate(data: unknown): ValidationResult {
  ...
  const errors = (validateSchema.errors ?? []).map((e: { instancePath: string; message?: string }) => ({
    path: e.instancePath || "/",
...
export function parse(json: string): PMFFile {
  let data: unknown;
  try {
